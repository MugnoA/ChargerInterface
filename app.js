...
function cacheDom() {
  ...
  dom.mEff      = document.getElementById("mEff");
  dom.effArc    = document.getElementById("effArc");
}
...
function updateEfficiency() {
  const pIn  = lastVoltIn * lastCurrIn;
  const pOut = lastVoltOut * lastCurrOut;
  const eff  = pIn > 0.1 ? Math.min((pOut / pIn) * 100, 100) : 0;
  dom.mEff.textContent = eff.toFixed(1);
  // SVG arc: circumference = 2*PI*42 ≈ 263.9
  const offset = 263.9 - (eff / 100) * 263.9;
  dom.effArc.setAttribute("stroke-dashoffset", offset.toFixed(1));
  // Color: teal if >80%, amber 50-80%, coral <50%
  if (eff >= 80)      dom.effArc.setAttribute("stroke", "var(--accent-teal)");
  else if (eff >= 50) dom.effArc.setAttribute("stroke", "var(--accent-amber)");
  else                dom.effArc.setAttribute("stroke", "var(--accent-coral)");
}
